...
describe("Auth Router - Unit tests", () => {
    ...
    it("Debe rechazar acceso a rutas protegidas sin token", async () => {
        const response = await request(server).get("/auth/protected");
        expect(response.statusCode).to.equal(401); 
        expect(response.body).to.have.property("error", "Token no encontrado. Acceso no autorizado.");
    });

    it("Debe permitir acceso a rutas protegidas con token", async () => {
        const response = await request(server)
            .get("/auth/protected")
            .set("Authorization", `Bearer ${token}`);
        expect(response.statusCode).to.equal(200);
        expect(response.body).to.have.property("email", "juanperez@example.com");
    });
});
